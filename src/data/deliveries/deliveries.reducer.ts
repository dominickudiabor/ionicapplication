...
export const deliveriesReducer = (state: ConfState, action: DeliveriesActions): ConfState => {
  switch (action.type) {
    ...
    case 'add-favorite': {
      return { ...state, favorites: [...(state.favorites), action.deliveryId] };
    }
    case 'remove-favorite': {
      return { ...state, favorites: [...(state.favorites).filter(x => x !== action.deliveryId)] };
    }
